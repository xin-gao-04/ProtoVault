...
| P7 双向同步 | MVP 与持久恢复完成 / 可视合并待深化 | Header 内容 hash、保存前冲突保护、保存后重扫、外部修改提示、SQLite 最后有效 IR、草稿保留/舍弃选择 | 源码编辑不会静默覆盖外部修改；进程重启后损坏 Header 仍可恢复最后有效 IR；完整 Disk/Base/Draft 可视三方合并待实现 |
...
1. 已建立 SQLite 增量索引、重启可复用 Header IR cache 和持久最后有效 IR。
2. 已建立 Struct、Enum、Field、枚举值的稳定身份重关联，并在语义 Diff 中报告改名。
3. 已加入扫描代次、AbortSignal 和 Clang 子进程终止，旧扫描不会覆盖新结果。
4. 已把 AI Assistant 从主入口拆为按需 chunk；其余大视图继续渐进拆分。
5. 已将外部变更操作区分为“保留草稿导入”和“舍弃草稿导入”，保存仍有 expected hash 防覆盖。
6. 尚需完成 Canonical IR/C++ Parser Service 收敛、完整 Disk/Base/Draft 可视合并、远端 Git、多协议包合成和签名/升级通道。
